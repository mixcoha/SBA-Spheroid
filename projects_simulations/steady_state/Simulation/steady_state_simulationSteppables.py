...
OUTPUT_DIR = os.path.join(os.getcwd(), "SimulationOutputs")
os.makedirs(OUTPUT_DIR, exist_ok=True)

# Configurar el logger para que guarde dentro de SimulationOutputs
logging.basicConfig(
...
class MutationSteppable(SteppableBasePy):
    ...
    def update_condition_counters(self, cell, o2_conc, glc_conc, lac_conc):
        ...
        if cell.type == CELL_TYPE_PROL and (o2_THRESHOLD_HIPO <= o2_conc <= o2_THRESHOLD and glc_THRESHOLD_HIPO <= glc_conc <= glc_THRESHOLD):
            ...
            logging.info(f"⚠️ Célula {cell.id} (PROL) puede volverse RESE en {MCS_4_CHANGE_PHE - conditions['low_o2_low_glu_prol_to_rese']} MCS")
        else:
        ...
        if cell.type == CELL_TYPE_RESE and self.env.is_stressed(cell):
            ...
            logging.info(f"⚠️ Célula {cell.id} (RESE) en estrés → puede volverse INVA en {MCS_4_CHANGE_PHE - conditions['low_o2_low_glu_rese_to_inva']} MCS")
        else:
        ...
        if cell.type == CELL_TYPE_RESE:
            if self.env.is_optimal(cell):
                ...
                logging.info(f"🟢 Célula {cell.id} (RESE) en ambiente óptimo: revertirá a PROL en {MCS_4_CHANGE_PHE - conditions['high_o2_high_glu_rese_to_prol']} MCS")
            elif o2_conc > o2_THRESHOLD_HIPO and glc_conc > glc_THRESHOLD_HIPO:
                conditions['high_o2_high_glu_rese_to_prol'] += 1
                logging.info(f"🟢 Célula {cell.id} (RESE) en buenas condiciones: revertirá a PROL en {MCS_4_CHANGE_PHE - conditions['high_o2_high_glu_rese_to_prol']} MCS")
            else:
        ...
        if cell.type == CELL_TYPE_INVA:
            if self.env.is_optimal(cell):
                ...
                logging.info(f"🟢 Célula {cell.id} (INVA) en ambiente óptimo: revertirá a RESE en {MCS_4_CHANGE_PHE - conditions['high_o2_high_glu_inva_to_rese']} MCS")
            elif o2_conc >= o2_THRESHOLD_HIPO and glc_conc >= glc_THRESHOLD_HIPO:
                conditions['high_o2_high_glu_inva_to_rese'] += 1
                logging.info(f"🟢 Célula {cell.id} (INVA) en buenas condiciones: revertirá a RESE en {MCS_4_CHANGE_PHE - conditions['high_o2_high_glu_inva_to_rese']} MCS")
            else:
